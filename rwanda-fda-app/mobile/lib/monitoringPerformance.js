...
export function isPerformancePayloadLooksValid(payload) {
  ...
  if (data.applications_summary != null && typeof data.applications_summary === 'object') return true;
  if (data.staff != null && typeof data.staff === 'object' && data.staff.staff_id != null) return true;
...
export async function fetchMonitoringPerformance(opts) {
  ...
  const urls = api.performanceGetUrlVariants(effectiveStaffId, 'all', tokenValue);
  ...
  const tryReturn = (res, raw) => {
    ...
    const okBody =
      res.ok && (isApiSuccess(payload) || isPerformancePayloadLooksValid(payload));
