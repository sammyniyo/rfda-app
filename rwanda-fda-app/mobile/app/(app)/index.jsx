...
function statusTone(isDark, status, t) {
  const s = String(status || "").toLowerCase();
  if (s === "delayed")
    return {
      bg: isDark ? "#2a1116" : "#fff1f2",
      color: colors.danger,
      label: t("behindSchedule"),
    };
  if (s === "tobedelayed")
    return {
      bg: isDark ? "#2b1f0b" : "#fff7ed",
      color: colors.warning,
      label: t("needsAttention"),
    };
  return {
    bg: isDark ? "#0f2a22" : "#ecfdf5",
  ...
  return (
    <View style={styles.reportStatRow}>
      <View style={styles.reportStatLeft}>
        ...
        <Text style={[styles.reportStatLabel, { color: textMuted }]}>
          {label}
        </Text>
      </View>
      <Text
  ...
  const inner = (
    <View style={styles.kpiTileRow}>
      ...
      <View style={styles.kpiTileTextCol}>
        <Text
          style={[
            dense ? styles.kpiTileValueDense : boostValue ? styles.kpiTileValueBoost : styles.kpiTileValue,
            tabularNumberStyle,
            { color: valueColor },
          ]}
          numberOfLines={1}
          adjustsFontSizeToFit
          minimumFontScale={dense ? 0.62 : boostValue ? 0.8 : 0.7}
        >
          {value}
        </Text>
        <Text
          style={[styles.kpiTileLabel, { color: textMuted }]}
          numberOfLines={2}
        >
          {label}
        </Text>
...
function ApplicationsMixBar({ appReport, trackColor, legendColor, t }) {
  const den = Math.max(1, Number(appReport.assignments) || 1);
  const parts = [
    {
      key: "d",
      n: Number(appReport.delayed) || 0,
      c: colors.danger,
      label: t("behindSchedule"),
    },
    {
      key: "r",
      n: Number(appReport.atRisk) || 0,
      c: colors.warning,
      label: t("needsAttention"),
    },
    {
      key: "a",
      n: Number(appReport.active) || 0,
      c: colors.fdaBlue,
      label: t("activeWord"),
    },
    {
      key: "c",
      n: Number(appReport.completed) || 0,
      c: colors.success,
      label: t("doneWord"),
    },
  ];
        ...
        {totalW < 0.001 ? (
          ...
          parts.map((p, i) => {
            ...
            return (
              <View
                key={p.key}
                style={[styles.mixBarSeg, { flex: w, backgroundColor: p.c }]}
              />
            );
          })
        )}
        ...
        {parts
          ...
          .map((p) => (
            <View key={p.key} style={styles.mixLegendItem}>
              ...
              <Text style={[styles.mixLegendText, { color: legendColor }]}>
                {p.label}
              </Text>
            </View>
          ))}
  ...
  return (
    <View style={styles.taskCompletionRow}>
      <View
        style={[
          styles.taskRing,
          { borderColor: trackColor, backgroundColor: ringFillBg },
        ]}
      >
        <Text
          style={[styles.taskRingPct, tabularNumberStyle, { color: textMain }]}
        >
          {completionRate}%
        </Text>
        <Text style={[styles.taskRingCaption, { color: textMuted }]}>
          complete
        </Text>
      </View>
      <View style={styles.taskCompletionRight}>
        <View
          style={[styles.taskProgressTrack, { backgroundColor: trackColor }]}
        >
          <View
            style={[
              styles.taskProgressFill,
              { width: `${barPct}%`, backgroundColor: ringColor },
            ]}
          />
        </View>
...
export default function Dashboard() {
  ...
  const tasks = useMemo(
    () =>
      rawTasks.map((t, index) => {
        const n = normalizeTaskFromPerformance(t, index);
        return {
          id: n.id,
          title: n.title,
          status: n.status,
  ...
  const apps = useMemo(
    () =>
      rawApps.map((a, index) => {
        ...
        const timelineStatus = canonicalApplicationTimeline({
          ...a,
          is_completed,
          is_active,
        });
  ...
  const openTasks = tasks.filter((t) => !isTaskDoneStatus(t.status)).length;
  const completedTasks = tasks.filter((t) => isTaskDoneStatus(t.status)).length;
  ...
  const delayedAppsForKpi =
    apps.length > 0 ? delayedApps : Number(appSummary?.delayed ?? 0);
  const dueSoonTasks = tasks.filter((t) => {
    if (isTaskDoneStatus(t.status)) return false;
  ...
  const completionRate =
    tasks.length > 0
      ...
      : Number(taskSummary?.total) > 0
        ? Math.round(
            (Number(taskSummary?.completed ?? 0) / Number(taskSummary.total)) *
              100,
          )
        : 0;
  const displayName =
    String(user?.name || user?.email || "Staff").split(" ")[0] || "Staff";
  const notifItems = Array.isArray(notificationsQuery.data)
    ? notificationsQuery.data
    : [];
  const unreadAlerts = notifItems.filter((n) => !n?.read_at).length;
  const lastSyncedAt = [
    performanceQuery.lastSyncedAt,
    notificationsQuery.lastSyncedAt,
  ]
    .filter(Boolean)
    .sort()
    .reverse()[0];

  const riskAppsLive = apps.filter(
    (a) => a.timelineStatus === "tobedelayed",
  ).length;
  const activeAssignmentsLive =
    rawApps.length > 0
      ? rawApps.filter((a) => Boolean(a.is_active) && !Boolean(a.is_completed))
          .length
      : 0;
  const appReport = {
    assignments: Math.max(
      heroAppCount,
      Number(totalAppAssignments ?? appSummary?.total ?? 0),
    ),
    active:
      rawApps.length > 0
        ? activeAssignmentsLive
        : Number(appSummary?.active ?? 0),
    ...
    unique:
      appSummary?.unique_applications != null
        ? Number(appSummary.unique_applications)
        : null,
  };
  const taskTotalForReport =
    Number(totalTasksFromApi ?? taskSummary?.total ?? tasks.length) ||
    tasks.length ||
    0;
  const taskCompletedForReport =
    taskSummary?.completed != null
      ? Number(taskSummary.completed)
      : completedTasks;

  const topApps = apps.slice(0, 8);
  const topTasks = useMemo(() => {
    const open = tasks.filter((t) => !isTaskDoneStatus(t.status));
  ...
  function taskStatusLabel(status) {
    ...
    if (s === "completed" || s === "review") return t("doneWord");
    if (s === "in_progress") return t("activeWord");
    return t("stillOpen");
  }
  ...
  const openApplications = (filterKey) => {
    if (filterKey) {
      router.push(
        `/(app)/applications?filter=${encodeURIComponent(filterKey)}`,
      );
    } else {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      // useQuery already stores friendly error state for the UI banner
    } finally {
      setRefreshing(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (
      ...
      user?.dutyStation !== nextGroup
    ) {
      patch.group = nextGroup;
      patch.dutyStation = nextGroup;
    }
    if (
      typeof s.is_non_statute === "boolean" &&
      user?.is_non_statute !== s.is_non_statute
    ) {
      ...
      >
        ...
        <FadeInView delay={0} translateY={12}>
          ...
          >
            <LinearGradient
              colors={
                isDark ? header.gradientDark : ["#f3fcf7", "#eefbf4", "#ffffff"]
              }
              locations={[0, 0.5, 1]}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 1 }}
              ...
              <View style={styles.heroTopRow}>
                ...
                >
                  <Image
                    ...
                    style={styles.logoNatural}
                    resizeMode="contain"
                  />
                </View>
                <View style={styles.heroTextCol}>
                  <Text style={[styles.heroGreeting, { color: textMain }]}>
                    {t("dashboardHi")}, {displayName}
                  </Text>
                  <Text
                    style={[styles.heroTagline, { color: textMuted }]}
                    numberOfLines={2}
                  >
                    {user?.dutyStation
                      ? t("dashboardTaglineWithStation").replace(
                          "{station}",
                          user.dutyStation,
                        )
                      : t("dashboardTaglineDefault")}
                  </Text>
                  <Text style={[styles.heroMetaLine, { color: textMuted }]}>
                    ...
                    ·{" "}
                    {nowTick.toLocaleTimeString(undefined, {
                      hour: "numeric",
                      minute: "2-digit",
                    })}
                  </Text>
        ...
        <FadeInView delay={48} translateY={10}>
          ...
          >
            <Text style={[styles.kpiCardTitle, { color: textMuted }]}>
              {t("overview")}
            </Text>

            <Text style={[styles.kpiGroupLabel, { color: textMuted }]}>
              {t("applications")}
            </Text>
            ...
            <Text
              style={[
                styles.kpiGroupLabel,
                styles.kpiGroupLabelSpaced,
                { color: textMuted },
              ]}
            >
              {t("tasks")}
        ...
        <FadeInView delay={55} translateY={8}>
          <Text style={[styles.reportsSectionLabel, { color: textMuted }]}>
            {t("reports")}
          </Text>
          <View
            style={[
          ...
          >
            <View
              style={[styles.reportAccent, { backgroundColor: colors.fdaBlue }]}
            />
            <View style={styles.reportInner}>
              ...
              >
                <View style={styles.reportHeadRow}>
                  ...
                  <View style={styles.reportHeadText}>
                    <Text style={[styles.reportTitle, { color: textMain }]}>
                      {t("applications")}
                    </Text>
                    <Text style={[styles.reportSubtitle, { color: textMuted }]}>
                      {t("applicationsWorkingOn")}
                    </Text>
                  </View>
                  <Ionicons
                    name={reportAppsExpanded ? "chevron-up" : "chevron-down"}
                    size={22}
                    color={textMuted}
                  />
                </View>
                  ...
                  <Text style={[styles.reportPeek, { color: textMuted }]}>
                    ...
                    <Text
                      style={[
                        ...
                        {
                          color:
                            appReport.delayed > 0 ? colors.danger : textMain,
                        },
                      ]}
                    >
                    ...
                    <Text
                      style={[
                        ...
                        {
                          color:
                            appReport.atRisk > 0 ? colors.warning : textMain,
                        },
                      ]}
                    >
              ...
              {reportAppsExpanded ? (
                <>
                  ...
                  <Text style={[styles.reportHeroLabel, { color: textMuted }]}>
                    {t("totalAssignments")}
                  </Text>
                  <ApplicationsMixBar
                    appReport={appReport}
                  ...
                  >
                    ...
                    <ReportStatRow
                      ...
                      textMuted={textMuted}
                    />
                    {appReport.unique != null &&
                    appReport.unique !== appReport.assignments ? (
                      <ReportStatRow
                        icon="document-text-outline"
                        label={t("uniqueFilings")}
              ...
              >
                <Text
                  style={[styles.reportFooterText, { color: colors.fdaBlue }]}
                >
                  {t("openFullReport")}
                </Text>
                <Ionicons
        ...
        <FadeInView delay={75} translateY={8}>
          ...
          >
            ...
            <View style={styles.reportInner}>
              ...
              >
                <View style={styles.reportHeadRow}>
                  ...
                  <View style={styles.reportHeadText}>
                    ...
                    <Text style={[styles.reportSubtitle, { color: textMuted }]}>
                      {t("yourWorkloadDeadlines")}
                    </Text>
                  </View>
                  <Ionicons
                    name={reportTasksExpanded ? "chevron-up" : "chevron-down"}
                    size={22}
                    color={textMuted}
                  />
                </View>
              ...
              >
                <Text
                  style={[styles.reportFooterText, { color: colors.fdaGreen }]}
                >
                  {t("openFullReport")}
                </Text>
                <Ionicons
          ...
          >
            ...
            {topTasks.length === 0 ? (
              <Text
                style={[
                  styles.emptyText,
                  { color: textMuted, paddingVertical: 8 },
                ]}
              >
                {tasks.length === 0 ? t("noTasksAssigned") : t("noOpenTasks")}
              </Text>
            ) : (
              topTasks.map((t) => {
                const due = toDate(t.due_date);
                return (
                  ...
                  >
                    <View style={{ flex: 1 }}>
                      <Text
                        style={[styles.taskRowTitle, { color: textMain }]}
                        numberOfLines={2}
                      >
                        {t.title}
                      </Text>
                      <Text style={[styles.taskRowMeta, { color: textMuted }]}>
                        {due
                          ? `${t("duePrefix")} ${due.toLocaleDateString([], { day: "2-digit", month: "short" })}`
                          : t("noDueDate")}
                      </Text>
                    </View>
                    <View
        ...
        <FadeInView delay={170} translateY={8}>
          ...
          >
            ...
            <View style={[styles.tableHead, { borderColor }]}>
              <Text style={[styles.thRef, { color: textMuted }]}>
                {t("refShort")}
              </Text>
              <Text style={[styles.thStatus, { color: textMuted }]}>
                {t("status")}
              </Text>
              <Text style={[styles.thDays, { color: textMuted }]}>
                {t("daysLeft")}
              </Text>
            </View>
            {topApps.length === 0 ? (
              <Text style={[styles.emptyText, { color: textMuted }]}>
                {t("noApplicationRecords")}
              </Text>
            ) : (
              topApps.map((item) => {
                const expanded = Boolean(expandedRows[item.id]);
                const tone = statusTone(isDark, item.timelineStatus, t);
                return (
                  <View
                    key={item.id}
                    style={[styles.tableRowWrap, { borderColor }]}
                  >
                    <PressableScale
                    ...
                    >
                      <View style={styles.refWrap}>
                        ...
                        />
                        <Text
                          style={[styles.rowRef, { color: textMain }]}
                          numberOfLines={1}
                        >
                          {item.trackingNo}
                        </Text>
                      ...
                      >
                        ...
                        Number.isNaN(Number(item.daysRemaining))
                          ? "—"
                    ...
                    {expanded ? (
                      ...
                      >
                        <Text
                          style={[styles.expandedTitle, { color: textMain }]}
                          numberOfLines={2}
                        >
                          {item.applicant}
                        </Text>
                        <View style={styles.expandedMetaRow}>
                          <Text
                            style={[styles.metaLabel, { color: textMuted }]}
                          >
                            {t("stage")}:
                          </Text>
                          <Text style={[styles.metaValue, { color: textMain }]}>
                            {item.stage}
                          </Text>
                        </View>
                        <View style={styles.expandedMetaRow}>
                          ...
                          <Text style={[styles.metaValue, { color: textMain }]}>
                            {item.daysTaken} of {item.daysAllowed}
                          </Text>
                        </View>
                        <View style={styles.expandedMetaRow}>
                          <Text
